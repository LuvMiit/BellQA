...
public class BankPage {
    ...
    public int checkRatesPage(){
        ...
        exchangeTable = chromeDriver.findElement(By.xpath("//section[not (@style)]//table[@class='card-rates-table cards']"));
        ((JavascriptExecutor) chromeDriver).executeScript("arguments[0].scrollIntoView(true);", exchangeTable);

        return rowsInTable.size()-1;
    }
    ...
    public double currencyExchange(String currency){
        visibilityChecker.visibilityWait("//section[not (@style)]//tr[@class='card-rates-table__row']//td[@class='card-rates-table__cell card-rates-table__sale large-text']");
        visibilityChecker.visibilityWait("//section[not (@style)]//tr[@class='card-rates-table__row']//td[@class='card-rates-table__cell card-rates-table__purchase large-text']");
        int rowIndex = 0;
